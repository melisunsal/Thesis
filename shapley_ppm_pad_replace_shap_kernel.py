...
OUT_DIR = os.path.join(REPO_ROOT, "outputs", DATASET, f"shap_pad_replace_shap_kernel_batch_{BATCH_INDEX}")
...
def main():
    ...
    # Baseline mask is all zeros (all PADs in prefix8)
    ...
    background = np.zeros((1, L), dtype=np.float64)
    masker = shap.maskers.Independent(background)

    max_evals = int((L + 1) * N_PERMUTATIONS)

    ke = shap.KernelExplainer(f_from_masks, background, link="identity")  # link optional

    phi = ke.shap_values(np.ones((1, L), dtype=np.float64), nsamples=2 * L + 200)
    phi = np.array(phi, dtype=np.float64).reshape(-1)  # (L,)
    base_val = float(np.array(ke.expected_value).reshape(-1)[0])
